...
# 2. CSS 주입 (기존 스타일 유지 + 생키 애니메이션 클래스 정의)
st.markdown("""
    <style>
    .stApp { background-color: #2B2B2B !important; }
    .logo-container {
        position: absolute; top: -60px; right: 0px; z-index: 1001;
    }
    ...
    div[data-testid="stMetric"] * { color: #000000 !important; font-weight: bold !important; }
    div[data-testid="stMetricValue"] > div { font-size: 38px !important; font-weight: 900 !important; }
    h1, h2, h3, .stMarkdown p, .stMarkdown span { color: #FFFFFF !important; }
    ...
    /* Plotly 내부의 모든 link 파트를 강제로 타겟팅 */
    .sankey-link {
        stroke-dasharray: 10, 5 !important;
        stroke-opacity: 0.6 !important;
        animation: sankey-flow 1s linear infinite !important;
    }
    </style>
...
@st.cache_data
def get_funnel_data():
    steps = ["유입", "정보입력 완료", "설계동의", "본인인증", "차량선택", "보험료 산출", "사진등록", "피보험자 정보확인", "청약내용 확인", "결제화면 진입", "전자서명 진입", "가입완료"]
...
with tab_funnel:
    ...
    inflow, calc, complete = funnel_df.iloc[0]['사용자수'], funnel_df[funnel_df['단계'] == "보험료 산출"]['사용자수'].values[0], funnel_df.iloc[-1]['사용자수']
    ...
    for i in range(len(funnel_df)):
        step_name, current_count = funnel_df.iloc[i]['단계'], funnel_df.iloc[i]['사용자수']
        if i < len(funnel_df) - 1:
            conv_rate = (funnel_df.iloc[i+1]['사용자수'] / current_count) * 100
            labels.append(f"<b>{step_name}</b><br>{current_count:,}명<br><span style='color:#00FF00'>→ {conv_rate:.1f}%</span> | <span style='color:#FF4B4B'>↓ {100-conv_rate:.1f}%</span>")
        else: labels.append(f"<b>{step_name}</b><br>{current_count:,}명")

    fig_sankey = go.Figure(data=[go.Sankey(
        node = dict(pad = 30, thickness = 40, line = dict(color = "white", width = 1), label = labels, color = "#FF6600"),
        link = dict(source = list(range(len(labels) - 1)), target = list(range(1, len(labels))), value = funnel_df['사용자수'].tolist()[1:], color = "rgba(255, 102, 0, 0.2)")
    )])

    fig_sankey.update_layout(title_text="단계별 진입자 수 및 [전환율 | 이탈률] 흐름", font_size=12, font_color="white", paper_bgcolor='rgba(0,0,0,0)', height=700)
    
    # 🌟 [해결책] 차트 출력 시 고유 ID를 부여하고 자바스크립트로 강제 주입
    st.plotly_chart(fig_sankey, use_container_width=True, key="sankey_chart")

    # 브라우저 단에서 SVG 요소에 클래스를 강제로 입히는 스크립트
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        if (links.length > 0) {
            links.forEach(link => {
                ...
                link.style.animation = "sankey-flow 1s linear infinite";
            });
            // 한번 적용되면 감시 중단
            // observer.disconnect(); 
        }
